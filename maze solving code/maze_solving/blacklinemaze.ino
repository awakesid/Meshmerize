...
void maze()
{
  ...
  while (1)
  {
    ...
    {
      follow_Segment();
      // forward(motor1, motor2, 70);    // After reaching a intercetion follow the shortest path turn
      // delay(50);
    brake(motor1, motor2);           // After getting intersetion fist thing that we have to do                                     // Drive straight a bit. This helps us in case we entered the                                   // intersection at an angle
    forward(motor1, motor2, 50);
    delay(30);
    brake(motor1, motor2);
     delay(0.5);
...
void turn(char dir)
{
  ...
  {
    case'L':
      left(motor1, motor2, 360);
      qtra.readLineBlack(sensors1);
      while (sensors1[0] < thr[0])  ///change
      {
        line_position = qtra.readLineBlack(sensors1);    //Move left until left extreme sensor goes out of the Black line comes on white
      }
      left(motor1, motor2, 270);
      qtra.readLineBlack(sensors1);
      while (sensors1[0] > thr[0])
      {
        line_position = qtra.readLineBlack(sensors1);        // Again move left until it get's on black line
      }
      follow_segment1();    // Fast pid after turn to align bot quickly on line
      brake(motor1, motor2);
      break;
    case'R':
      right(motor1, motor2, 360);
      qtra.readLineBlack(sensors1);
      while (sensors1[7] < thr[7])
      {
        line_position = qtra.readLineBlack(sensors1);   //Move right until left extreme sensor goes out of the black line comes on white
      }
      right(motor1, motor2, 270);
      qtra.readLineBlack(sensors1);
      while (sensors1[7] > thr[7])
      {
